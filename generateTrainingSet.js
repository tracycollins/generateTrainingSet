...
for (const inputType of Object.keys(
  ...
  DEFAULT_TEST_MIN_TOTAL_MIN_TWEETS_TYPE_HASHMAP[inputType] =
    1 + Math.floor(0.1 * DEFAULT_MIN_TOTAL_MIN_TWEETS_TYPE_HASHMAP[inputType]);
}
...
for (const inputType of Object.keys(
  ...
  DEFAULT_TEST_MIN_TOTAL_MIN_PROFILE_TYPE_HASHMAP[inputType] =
    1 + Math.floor(0.1 * DEFAULT_MIN_TOTAL_MIN_PROFILE_TYPE_HASHMAP[inputType]);
}
...
let sum =
  configuration.maxTestCount.left +
  configuration.maxTestCount.neutral +
  configuration.maxTestCount.right;
...
configuration.DROPBOX.DROPBOX_CONFIG_FILE =
  process.env.DROPBOX_GTS_CONFIG_FILE || "generateTrainingSetConfig.json";
configuration.DROPBOX.DROPBOX_GTS_STATS_FILE =
  process.env.DROPBOX_GTS_STATS_FILE || "generateTrainingSetStats.json";
...
configuration.tempUserDataFolder = path.join(
...
configuration.userArchiveFile =
  hostname +
  "_" +
  statsObj.startTimeMoment.format(compactDateTimeFormat) +
  "_users.zip";
...
configuration.local.histogramsFolder = path.join(
  configHostFolder,
  "histograms"
);
...
configuration.default.histogramsFolder = path.join(
  configDefaultFolder,
  "histograms"
);
...
const saveGlobalHistogramsOnly = {
  name: "saveGlobalHistogramsOnly",
  alias: "H",
  type: Boolean,
};
const maxHistogramValue = {
  name: "maxHistogramValue",
  alias: "M",
  type: Number,
};
...
async function showStats(options) {
  ...
  statsObj.redis.keys =
    matchArray && matchArray !== undefined && matchArray[1] !== undefined
    ...
    console.log(
      chalkLog(
        MODULE_ID_PREFIX +
          ...
          " | L: " +
          categorizedUserHistogram.left +
          " | R: " +
          categorizedUserHistogram.right +
          " | N: " +
          categorizedUserHistogram.neutral +
          " | +: " +
          categorizedUserHistogram.positive +
          " | -: " +
          categorizedUserHistogram.negative +
          " | 0: " +
          categorizedUserHistogram.none
      )
    );
    ...
    console.log(
      chalkInfo(
        MODULE_ID_PREFIX +
          ...
          getTimeStamp() +
          " | START: " +
          getTimeStamp(statsObj.users.processed.startMoment) +
          " | ELSD: " +
          msToTime(statsObj.users.processed.elapsed) +
          " | ETC: " +
      ...
      console.log(
        chalkGreen(
          MODULE_ID_PREFIX +
            ...
            " MB" +
            " | TOTAL: " +
            statsObj.totalMbytes.toFixed(3) +
            " MB" +
            " | DST: " +
...
async function loadConfigFile(params) {
  ...
  try {
    ...
    if (loadedConfigObj.GTS_TEST_MODE !== undefined) {
      ...
      if (
        loadedConfigObj.GTS_TEST_MODE === true ||
        loadedConfigObj.GTS_TEST_MODE === "true"
      ) {
        newConfiguration.testMode = true;
      } else if (
        loadedConfigObj.GTS_TEST_MODE === false ||
        loadedConfigObj.GTS_TEST_MODE === "false"
      ) {
    ...
    if (loadedConfigObj.GTS_OFFLINE_MODE !== undefined) {
      ...
      if (
        loadedConfigObj.GTS_OFFLINE_MODE === false ||
        loadedConfigObj.GTS_OFFLINE_MODE === "false"
      ) {
        newConfiguration.offlineMode = false;
      } else if (
        loadedConfigObj.GTS_OFFLINE_MODE === true ||
        loadedConfigObj.GTS_OFFLINE_MODE === "true"
      ) {
    ...
    if (loadedConfigObj.GTS_SAVE_GLOBAL_HISTOGRAMS_ONLY !== undefined) {
      ...
      if (
        loadedConfigObj.GTS_SAVE_GLOBAL_HISTOGRAMS_ONLY === false ||
        loadedConfigObj.GTS_SAVE_GLOBAL_HISTOGRAMS_ONLY === "false"
      ) {
        newConfiguration.saveGlobalHistogramsOnly = false;
      } else if (
        loadedConfigObj.GTS_SAVE_GLOBAL_HISTOGRAMS_ONLY === true ||
        loadedConfigObj.GTS_SAVE_GLOBAL_HISTOGRAMS_ONLY === "true"
      ) {
    ...
    if (loadedConfigObj.GTS_QUIT_ON_COMPLETE !== undefined) {
      ...
      if (
        !loadedConfigObj.GTS_QUIT_ON_COMPLETE ||
        loadedConfigObj.GTS_QUIT_ON_COMPLETE === "false"
      ) {
    ...
    if (loadedConfigObj.GTS_DATA_ROOT_FOLDER !== undefined) {
      ...
      newConfiguration.tempUserDataFolder = path.join(
        newConfiguration.dataRootFolder,
        "trainingSets/users"
      );
      newConfiguration.userDataFolder = path.join(
        newConfiguration.dataRootFolder,
        "users"
      );
    }
    ...
    if (loadedConfigObj.GTS_MAX_GLOBAL_HISTOGRAM_USERS !== undefined) {
      ...
      newConfiguration.maxGlobalHistogramUsers =
        loadedConfigObj.GTS_MAX_GLOBAL_HISTOGRAM_USERS;
    }
    ...
    if (loadedConfigObj.GTS_TOTAL_MAX_TEST_COUNT !== undefined) {
      ...
      newConfiguration.totalMaxTestCount =
        loadedConfigObj.GTS_TOTAL_MAX_TEST_COUNT;
      newConfiguration.maxTestCount = {};
      newConfiguration.maxTestCount.left = parseInt(
        0.333333 * loadedConfigObj.GTS_TOTAL_MAX_TEST_COUNT
      );
      newConfiguration.maxTestCount.neutral = parseInt(
        0.333333 * loadedConfigObj.GTS_TOTAL_MAX_TEST_COUNT
      );
      newConfiguration.maxTestCount.right = parseInt(
        0.333333 * loadedConfigObj.GTS_TOTAL_MAX_TEST_COUNT
      );

      let sum =
        newConfiguration.maxTestCount.left +
        newConfiguration.maxTestCount.neutral +
        newConfiguration.maxTestCount.right;
    ...
    if (loadedConfigObj.GTS_MIN_TOTAL_MIN_TWEETS_TYPE_HASHMAP !== undefined) {
      ...
      newConfiguration.inputTypeMinTweetsHashMap =
        loadedConfigObj.GTS_MIN_TOTAL_MIN_TWEETS_TYPE_HASHMAP;
    }
    ...
    if (loadedConfigObj.GTS_MIN_TOTAL_MIN_PROFILE_TYPE_HASHMAP !== undefined) {
      ...
      newConfiguration.inputTypeMinProfileHashMap =
        loadedConfigObj.GTS_MIN_TOTAL_MIN_PROFILE_TYPE_HASHMAP;
    }
    ...
    if (loadedConfigObj.GTS_WAIT_VALUE_INTERVAL !== undefined) {
      ...
      newConfiguration.waitValueInterval =
        loadedConfigObj.GTS_WAIT_VALUE_INTERVAL;
    }
    ...
    if (loadedConfigObj.GTS_SAVE_FILE_MAX_PARALLEL !== undefined) {
      ...
      newConfiguration.saveFileMaxParallel =
        loadedConfigObj.GTS_SAVE_FILE_MAX_PARALLEL;
    }
    ...
    if (loadedConfigObj.GTS_MAX_SAVE_FILE_QUEUE !== undefined) {
      ...
      newConfiguration.maxSaveFileQueue =
        loadedConfigObj.GTS_MAX_SAVE_FILE_QUEUE;
    }
    ...
    if (loadedConfigObj.GTS_SAVE_FILE_BACKPRESSURE_PERIOD !== undefined) {
      ...
      newConfiguration.saveFileBackPressurePeriod =
        loadedConfigObj.GTS_SAVE_FILE_BACKPRESSURE_PERIOD;
    }
    ...
    if (loadedConfigObj.GTS_MAX_HISTOGRAM_VALUE !== undefined) {
      ...
      newConfiguration.maxHistogramValue =
        loadedConfigObj.GTS_MAX_HISTOGRAM_VALUE;
    }
  ...
  try {
    ...
    if (!empty(user.profileHistograms.sentiment)) {
      if (user.profileHistograms.sentiment.magnitude !== undefined) {
        ...
        statsObj.normalization.magnitude.max = Math.max(
          statsObj.normalization.magnitude.max,
          user.profileHistograms.sentiment.magnitude
        );
      }
      ...
      if (user.profileHistograms.sentiment.score !== undefined) {
        ...
        statsObj.normalization.score.max = Math.max(
          statsObj.normalization.score.max,
          user.profileHistograms.sentiment.score
        );
        statsObj.normalization.score.min = Math.min(
      ...
      if (user.profileHistograms.sentiment.comp !== undefined) {
        statsObj.normalization.comp.max = Math.max(
          statsObj.normalization.comp.max,
          user.profileHistograms.sentiment.comp
        );
        statsObj.normalization.comp.min = Math.min(
          statsObj.normalization.comp.min,
          user.profileHistograms.sentiment.comp
        );
      }
      ...
      console.log(
        chalkLog(
          MODULE_ID_PREFIX +
            ...
            statsObj.normalization.score.max +
            " | MAG min: " +
            statsObj.normalization.magnitude.min +
            " max: " +
            statsObj.normalization.magnitude.max +
            " | COMP min: " +
            statsObj.normalization.comp.min +
...
async function categorizeUser(params) {
  try {
    ...
    const userPropertyPickArray =
      params.userPropertyPickArray || configuration.userPropertyPickArray;

    userIndex += 1;

    // await tcUtils.updateGlobalHistograms({user: user, verbose: true});

    const subUser = pick(user, userPropertyPickArray);
...
async function cursorDataHandler(params) {
  ...
  if (user.tweetHistograms.friends === null) {
    ...
    await global.wordAssoDb.User.updateOne(
      { nodeId: user.nodeId },
      { $unset: { "tweetHistograms.friends": "" } }
    );
  }

  if (user.profileHistograms.friends || user.tweetHistograms.friends) {
    if (
      user.tweetHistograms.friends !== undefined &&
      user.tweetHistograms.friends
    ) {
      debug(
        chalkAlert(
      ...
      console.log(
        chalkWarn(
          MODULE_ID_PREFIX +
            ...
            " | PF HIST: " +
            (user.profileHistograms
              ? Object.keys(user.profileHistograms).length
              : 0) +
            " | TW HIST: " +
            (user.tweetHistograms
              ? Object.keys(user.tweetHistograms).length
              : 0) +
            " | CAT: " +
...
async function categoryCursorStream(params) {
  ...
  const cursorBatchSize =
    params.cursorBatchSize || configuration.cursorBatchSize;
  ...
  console.log(
    chalkGreen(
      MODULE_ID_PREFIX +
        ...
        " | SAVE BACK PRESSURE PERIOD: " +
        configuration.saveFileBackPressurePeriod +
        " | SFQ PARALLEL: " +
        configuration.saveFileMaxParallel +
        " | TOTAL USERS: " +
        statsObj.users.grandTotal
    )
  );
...
const allCursorsComplete = () =>
  Object.values(categoryCursorHash).every(
    (categoryCursor) => categoryCursor.complete
  );
...
async function initialize(cnf) {
  ...
  if (process.env.GTS_QUIT_ON_COMPLETE !== undefined) {
    ...
    if (
      !process.env.GTS_QUIT_ON_COMPLETE ||
      process.env.GTS_QUIT_ON_COMPLETE === false ||
      process.env.GTS_QUIT_ON_COMPLETE === "false"
    ) {
...
async function generateGlobalTrainingTestSet() {
  ...
  configuration.userGlobalHistogramProbability =
    statsObj.users.grandTotal > 0
      ? configuration.maxGlobalHistogramUsers / statsObj.users.grandTotal
      : 0;
  ...
  console.log(
    chalkBlueBold(
      MODULE_ID_PREFIX +
        ...
        " | N: " +
        statsObj.userCategoryTotal.neutral +
        " | R: " +
        statsObj.userCategoryTotal.right
    )
  );
  console.log(
    chalkBlueBold(
  ...
  if (configuration.testMode) {
    ...
    statsObj.users.grandTotal = Math.min(
      statsObj.users.grandTotal,
      configuration.totalMaxTestCount
    );
    statsObj.users.processed.remain = statsObj.users.grandTotal;
    console.log(
      chalkAlert(
  ...
  for (const category of ["left", "neutral", "right"]) {
    ...
    await categoryCursorStream({
      category: category,
      ignored: false,
      cursorParallel: configuration.cursorParallel,
    });
...
setTimeout(async function () {
  try {
    ...
    if (
      configuration.enableCreateUserArchive &&
      !configuration.saveGlobalHistogramsOnly
    ) {
      console.log(
        chalkAlert(
          MODULE_ID_PREFIX +
      ...
      statsObj.runSubFolder = path.join(
        configuration.userArchiveFolder,
        statsObj.runId
      );
    ...
    if (configuration.enableCreateUserArchive) {
      ...
      for (const subFolderIndexString of [...subFolderSet]) {
        const folder = path.join(
          configuration.tempUserDataFolder,
          "data",
          subFolderIndexString
        );
        const archivePath = path.join(
          statsObj.runSubFolder,
          "userArchive" + subFolderIndexString + ".zip"
        );
    ...
    const inputTypeMinProfileHashMap = configuration.testMode
      ? configuration.testInputTypeMinProfileHashMap
      : configuration.inputTypeMinProfileHashMap;
    const inputTypeMinTweetsHashMap = configuration.testMode
