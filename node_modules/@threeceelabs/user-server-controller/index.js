...
class uscEmitter extends EventEmitter {};

let UserServerController = function(app_name){

	console.log("USER SERVER CONTROLLER | APP NAME: " + app_name);
...
UserServerController.prototype.ready = function () {
	return ready;
};
...
UserServerController.prototype.findOneUser = function (user, params, callback) {
	...
	if ((user.screenName !== undefined) && user.screenName) { 
		update["$set"].screenName = user.screenName.toLowerCase();
		update["$set"].screenNameLower = user.screenName.toLowerCase();
	}
	...
	if ((user.lastTweetId !== undefined) && user.lastTweetId){
		update["$set"].lastTweetId = user.lastTweetId;
	}

	if ((user.ignored !== undefined) && user.ignored){
		update["$set"].ignored = user.ignored;
	}
	...
	if ((user.statusId !== undefined) && user.statusId) { 
		update["$set"].statusId = user.statusId;
	}

	if ((user.previousStatusId !== undefined) && user.previousStatusId) { 
	...
	if ((user.bannerImageAnalyzed !== undefined) && user.bannerImageAnalyzed) { 
	...
	if ((user.languageAnalyzed !== undefined) && user.languageAnalyzed) { 
		update["$set"].languageAnalyzed = user.languageAnalyzed;
	}
	if ((user.languageAnalysis !== undefined) && user.languageAnalysis) { 
...
UserServerController.prototype.findCategorizedUsersCursor = function (params, callback) {
	...
	const defaultProjection = "nodeId screenName name lang description location category categoryAuto mentions friendsCount followersCount statusesCount ignored histograms following threeceeFollowing";
...
UserServerController.prototype.convertRawUser = function (params, callback) {
	...
	user.ignored = params.user.ignored;
  ...
  user.lastTweetId = (params.lastTweetId) ? params.lastTweetId : false;
	...
	user.status = params.user.status || null;
...
UserServerController.prototype.updateHistograms = function (params, callback) {
	...
	User.findOne({ nodeId: params.user.nodeId }).exec(function(err, user) {
      ...
      async.each(paramsHistogramTypes, function(type, cb0){
      	...
      	if (user.rate === undefined) { user.rate = 0; }
      	if (params.user.lang) { user.lang = params.user.lang; }
      	if (params.user.createdAt) { user.createdAt = params.user.createdAt; }
      	if (params.user.ignored) { user.ignored = params.user.ignored; }
      	...
      	if (params.user.location) { user.location = params.user.location; }
